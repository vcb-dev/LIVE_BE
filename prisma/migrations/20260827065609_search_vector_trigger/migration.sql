...
CREATE OR REPLACE FUNCTION script_blocks_search_vector_update()
  RETURNS trigger
  LANGUAGE plpgsql
AS $$
BEGIN
  NEW.search_vector := to_tsvector(
    'simple',
    public.unaccent(coalesce(NEW.title, '') || ' ' || NEW.content)
  );
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER script_blocks_search_vector_tg
  ...
  FOR EACH ROW
  EXECUTE FUNCTION script_blocks_search_vector_update();
